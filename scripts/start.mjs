...
async function main() {
  ...
  if (location === 'root') {
    ...
    const { selectedPackage } = await inquirer.prompt([
      { type: 'list', name: 'selectedPackage', message: chalk.green('选择一个包:'), choices: packages.map(p => ({ name: p.name, value: p })) },
    ]);
    ...
    const { script } = await inquirer.prompt([
      { type: 'list', name: 'script', message: chalk.green(`选择包 "${selectedPackage.name}" 中的一个脚本:`), choices: scriptChoices },
    ]);
